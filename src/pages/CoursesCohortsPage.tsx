      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter">
        <div className="bg-surface-container-lowest p-stack-md border border-outline-variant rounded-lg shadow-xs">
          <div className="flex justify-between items-start mb-stack-md">
            ...
            <span className="text-xs font-bold text-emerald-800 bg-emerald-100 px-2 py-1 rounded">Active</span>
          </div>
        ...
        <div className="bg-surface-container-lowest p-stack-md border border-outline-variant rounded-lg shadow-xs">
          <div className="flex justify-between items-start mb-stack-md">
            ...
            <span className="text-xs font-bold text-primary bg-secondary-container px-2 py-1 rounded">Running</span>
          </div>
        ...
        <div className="bg-surface-container-lowest p-stack-md border border-outline-variant rounded-lg shadow-xs">
          <div className="flex justify-between items-start mb-stack-md">
            ...
            <span className="text-xs font-bold text-on-tertiary-container bg-surface-container px-2 py-1 rounded">
              {capacityPercent}% Enrolled
            </span>
          </div>
          <p className="font-body-sm text-body-sm text-secondary mb-unit">Total Seat Capacity</p>
          <div className="flex items-baseline gap-2">
            <h3 className="font-display text-display font-bold text-on-surface">{totalEnrolled} / {totalSeats}</h3>
          ...
          <div className="w-full bg-surface-container-high rounded-full h-1.5 mt-2 overflow-hidden">
            <div className="bg-primary h-1.5 rounded-full" style={{ width: `${capacityPercent}%` }}></div>
          </div>
        </div>
      </div>
                ...
                >
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="flex items-center gap-1 text-xs font-bold text-amber-800 bg-amber-100 px-2 py-0.5 rounded">
                    ...
                    <div className="flex items-center gap-2">
                      {currentUser?.role === 'super_admin' ? (
                        <button
                          ...
                          className="px-2.5 py-1 rounded border border-outline-variant hover:border-primary text-secondary hover:text-primary font-label-md text-xs font-semibold transition-colors flex items-center gap-1"
        ...
        {activeTab === 'cohorts' && (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[700px]">
              ...
              <tbody className="font-data-tabular text-body-md text-on-surface divide-y divide-outline-variant/50">
                {filteredCohorts.map((coh, index) => {
                  const percent = Math.round((coh.enrolledCount / coh.maxCapacity) * 100);
                  return (
                    <tr
                      key={coh.id}
